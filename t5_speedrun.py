...
train_encodings = tokenizer(train_source, truncation = False, padding=True)
train_decodings = tokenizer(train_target, truncation = False, padding=True)

test_encodings = tokenizer(test_source, truncation = True, padding=True)
...
for epoch in range(epochs):
    ...
    model.save_pretrained(f'/home4/s4236599/speedrun_trial/t5/t5_epoch{epoch+1}_{test_loss}')
loss_df = pd.DataFrame(list(zip(train_loss_his, test_loss_his)))
loss_df.to_csv("/home4/s4236599/speedrun_trial/t5_loss.csv", index = False)
